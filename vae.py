...
class solver:
	...
	def __init__(self,path,num_vertices,dec_path):
		self.mode='train'
		...
		self.angle_num_types=16
		self.m_dim = self.atom_num_types
		self.b_dim = self.bond_num_types
		self.dihedral_dim=self.angle_num_types
		self.f_dim = 0
		self.dropout=0
	...
	def build_model(self):
		self.encoder = EncoderVAE(self.d_conv_dim, self.m_dim, self.b_dim - 1, self.z_dim, self.dihedral_dim,
		...
		self.decoder = Generator(self.g_conv_dim, self.z_dim, self.vertexes, self.bond_num_types,
                                 self.atom_num_types, self.dihedral_dim, self.dropout_rate).to(self.device)
	...
	def get_reconstruction_loss(self, n_hat, n, e_hat, e, d_hat, d):
		...
		d_loss = torch.nn.CrossEntropyLoss(reduction='mean')(d_hat.view(-1, self.dihedral_dim), d.view(-1))
		reconstruction_loss = n_loss + e_loss + d_loss
		return reconstruction_loss
	...
	def train_or_valid(self, epoch_i, train_val_test='val'):
		...
		for a_step in range(the_step):
			...
			if train_val_test == 'train' or train_val_test == 'val':
				...
				x = torch.tensor(nodes).to(self.device).long()
				d = torch.tensor(angles).to(self.device).long()
			
				 # Dihedral_Angles.
			
				a_tensor = self.label2onehot(a, self.b_dim)
				x_tensor = self.label2onehot(x, self.m_dim)
				d_tensor = self.label2onehot(d, self.dihedral_dim)
		
			if train_val_test == 'train' or train_val_test == 'val':
				z, z_mu, z_logvar = self.encoder(a_tensor, None, x_tensor, d_tensor)
				
			
			edges_logits, nodes_logits, angles_logits = self.decoder(z)

			if train_val_test == 'train' or train_val_test == 'val':

				recon_loss = self.get_reconstruction_loss(nodes_logits, x, edges_logits, a, angles_logits, d)
		...
		if (epoch_i==19 or epoch_i==29 or epoch_i==39 or epoch_i==49):
			# z = self.sample_z(self.batch_size)
			# z = torch.from_numpy(z).to(self.device).float()
...
sol=solver('/scratch/trahman2/modified_16.npy',16,"/scratch/trahman2/saved_models/models_16/")
