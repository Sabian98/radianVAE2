...
class Generator(nn.Module):
    ...
    def __init__(self, conv_dims, z_dim, vertexes, edges, nodes, angles, dropout_rate):
        ...
        self.angles_layer = nn.Linear(conv_dims[-1], vertexes * angles)
    ...
    def forward(self, x):
        ...
        angles_logits=self.angles_layer(output)
        angles_logits = self.dropoout(angles_logits.view(-1, self.vertexes, self.angles))

        return edges_logits, nodes_logits, angles_logits


class EncoderVAE(nn.Module):
    """VAE encoder sharing part."""
    def __init__(self, conv_dim, m_dim, b_dim, z_dim,angle_dim, with_features=False, f_dim=0, dropout_rate=0.):
        super(EncoderVAE, self).__init__()

        graph_conv_dim, aux_dim, linear_dim = conv_dim
        # discriminator
        #conv_dim=d_conv_dim[[128, 64], 128, [128, 64]];m_dim = self.atom_num_types;b_dim = self.bond_num_types
        ...
        self.gcn_layer_2 = GraphConvolution(angle_dim, graph_conv_dim, b_dim, with_features, f_dim, dropout_rate)
        self.agg_layer_2 = GraphAggregation(graph_conv_dim[-1]+angle_dim, aux_dim, torch.nn.Tanh(), with_features, f_dim,
                                          dropout_rate)
    ...
    def forward(self, adj, hidden, node, angles, activation=None):
        ...
        h2 = self.gcn_layer_2(angles, adj, hidden)
        h2 = self.agg_layer_2(h2, angles, hidden)
        h = h.add(h2)
